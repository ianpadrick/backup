Trigger deal_CreateApprovalHistory on LLC_CDS__Deal_Facility__c (before update) {
    ...
    for (Integer i = 0 ; i < trigger.new.size(); i++) {
        if (trigger.isUpdate && trigger.new[i].Last_Approval_Status__c != null) {
            if ((trigger.new[i].Last_Approval_Status__c == 'Approved') || (trigger.new[i].Last_Approval_Status__c == 'Rejected') || (trigger.new[i].Last_Approval_Status__c == 'Removed')) {
                dealIds.add(trigger.new[i].Id);
            }
        }
        for (LLC_CDS__Deal_Facility__c deal : trigger.new) {
            if (deal.Last_Approval_Status__c != null) {
                deal.Last_Approval_Status__c = '';
            }
        }
    }

    if (dealIds.size() > 0) {
        Map<Id,ProcessInstance> processMap = new Map<Id, ProcessInstance>([
            SELECT
            Id,
            CompletedDate,
            ElapsedTimeInDays,
        ...
        FROM
            ProcessInstance
        WHERE
            TargetObjectId IN : dealIds
        AND
            ((Status != 'Started') AND (Status != 'Submitted'))]);

        for (ProcessInstance psi : processMap.values()) {
            for (ProcessInstanceStep ppt : psi.Steps) {
                Integer historyCounter = [SELECT COUNT() FROM Approval_History__c WHERE Deal_Facility__c =: psi.TargetObjectId AND ((Actual_Approver__c =: psi.LastActor.Name) AND (Date__c =: psi.CompletedDate) AND (Comments__c =: ppt.Comments) AND (Assigned_To__c =: ppt.OriginalActor.Name) AND (Status__c =: ppt.StepStatus))];
                if (historyCounter == 0) {
